...
void insertLast(Head* phead, int data)
{
	...
	newNode->data = data;//3
	newNode->index = phead->size +1;//풍선 번호 1 :: 리스트 크기 +1
	...
	phead->size++; //관리자이기 때문에 phead 를 사용한다고 함니다..
}

void deleteNode(Head* phead){
	DNode* head = phead->link;//1번 풍선 3의 값
